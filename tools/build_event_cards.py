...
CARDS = [
    # ---- 14 routine ------------------------------------------------------
    # 后期现金可达数十亿, 这些「喝奶茶 / 养猫 / 换咖啡机」的琐事 cap 砍到几万级,
    # 否则按 pct 撞 cap 会算出 100w 量级, 严重出戏。养猫 / 咖啡机 / 实习生再加
    # max_triggers 限制只触发 1-2 次, 不再反复刷屏 (见 design/事件系统设计.md §4.2.1)。
    routine("routine_all_hands", "周一全员大会",
        "周一早上九点, 全员盯着你, 等一句开场白。",
        [("boost_morale", "请全员喝奶茶 (小额开销)",
            [(SPEND, {"pct": 0.004, "floor": 800, "cap": 100000})]),
         ("adjourn", "三句话散会", [])],
        requires_staff_min=1),
    routine("routine_office_pet", "办公室来了只流浪猫",
        "一只橘猫赖在工位上不走, 实习生提议让它当『首席摸鱼官』。",
        [("adopt", "正式领养, 配猫粮预算",
            [(SPEND, {"pct": 0.003, "floor": 600, "cap": 60000})]),
         ("shoo", "礼貌请它出门", [])],
        max_triggers=2),
    routine("routine_coffee_machine", "咖啡机彻底罢工了",
        "唯一的咖啡机冒了一阵青烟后再也不响了, 工程师们眼神涣散。",
        [("buy_fancy", "换一台高级意式咖啡机",
            [(SPEND, {"pct": 0.006, "floor": 1200, "cap": 100000})]),
         ("instant", "改喝速溶, 主打一个朴素", [])],
        requires_staff_min=1, max_triggers=2),
    routine("routine_team_building", "该团建了",
        "HR 在群里发了个问号, 大家都懂——团建该安排了。",
        [("fancy", "豪华团建, 团队嗨翻",
            [(SPEND, {"pct": 0.012, "floor": 2500, "cap": 5000000})]),
         ("cheap", "楼下烧烤摊打卡",
            [(SPEND, {"pct": 0.002, "floor": 300, "cap": 200000})])],
        requires_staff_min=1),
